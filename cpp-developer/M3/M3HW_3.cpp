...
int main() {
    const int beginHeight = 100; // начальная высота бамбука в сантиметрах
    const int dailyGrowth = 50;  // ежедневный рост бамбука в сантиметрах
    const int nightFade = 20;    // размер съедаемого гусеницами бамбука за одну ночь в сантиметрах

    // Вычисляем высоту бамбука к середине третьего дня
    int heightAtMidThirdDay = beginHeight + (dailyGrowth - nightFade) * 2 + dailyGrowth / 2;

    // Выводим результат
    cout << "Высота бамбука к середине третьего дня: " << heightAtMidThirdDay << " см" << endl;
